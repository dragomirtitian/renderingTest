...
namespace RenderingTest.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            var xx = System.Web.Hosting.HostingEnvironment.ApplicationHost.GetConfigMapPathFactory().Create(@"\", @"c:\users\titian.dragomir\documents\visual studio 2013\Projects\RenderingTest\RenderingTest\");
            var x = Thread.GetDomain().GetData(".appDomain");
            x = Thread.GetDomain().GetData(".appId");
            x = Thread.GetDomain().GetData(".appPath");
            x = Thread.GetDomain().GetData(".appVPath");
            x = Thread.GetDomain().GetData(".domainId");
            var r = new RequestData()
            {
                AcceptTypes = this.Request.AcceptTypes,
                AnonymousID = this.Request.AnonymousID,
                ApplicationPath = this.Request.ApplicationPath,
                AppRelativeCurrentExecutionFilePath = this.Request.AppRelativeCurrentExecutionFilePath,
                //X = this.Request.Browser,
                ContentLength = this.Request.ContentLength,
                ContentType = this.Request.ContentType,
                CurrentExecutionFilePath = this.Request.CurrentExecutionFilePath,
                FilePath = this.Request.FilePath,
                HttpMethod = this.Request.HttpMethod,
                IsAuthenticated = this.Request.IsAuthenticated,
                IsLocal = this.Request.IsLocal,
                IsSecureConnection = this.Request.IsSecureConnection,
                Path = this.Request.Path,
                PathInfo = this.Request.PathInfo,
                PhysicalApplicationPath = this.Request.PhysicalApplicationPath,
                PhysicalPath = this.Request.PhysicalPath,
                ...
                UrlReferrer = this.Request.UrlReferrer == null ? null : this.Request.UrlReferrer.ToString(),
                UserAgent = this.Request.UserAgent,
                UserHostAddress = this.Request.UserHostAddress,
                UserHostName = this.Request.UserHostName,
                UserLanguages = this.Request.UserLanguages,
            };

            var sw = new StringWriter();
            new XmlSerializer(typeof(RequestData)).Serialize(sw, r);

            var sw2 = new StringWriter();
            new XmlSerializer(typeof(HttpBrowserCapabilitiesData)).Serialize(sw2, new HttpBrowserCapabilitiesData(this.Request.Browser));
